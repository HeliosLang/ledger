...
export class MintingPolicyHash extends ScriptHash {
	...
	constructor(bytes) {
		...
		if (!(this.bytes.length == 28 || this.bytes.length == 0)) {
            throw new Error(`expected 0 or 28 bytes for MintingPolicyHash, got ${this.bytes.length}`)
        }
	}
	 ...
	 */
	static from(arg) {
		return arg instanceof MintingPolicyHash ? arg : new MintingPolicyHash(arg);
	}
