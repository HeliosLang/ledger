...
export class Address {
	...
	constructor(bytesOrBech32String) {
        ...
        if (!(this.bytes.length == 29 || this.bytes.length == 57)) {
            throw new Error(`expected 29 or 57 bytes for Address, got ${this.bytes.length}`)
        }
	}
    ...
    static fromUplcData(data, isTestnet = config.isTestnet) {
        ...
        const credData = ConstrData.expect(data.fields[0], "invalid Crendetial UplcData within Address").expectFields(1, "invalid Crendetial UplcData within Address")
        const stakData = ConstrData.expect(data.fields[1], "invalid StakingCredential UplcData within Address")
        ...
        if (stakData.tag == 1) {
			...
			stakData.expectFields(1, "invalid StakingCredential UplcData content within Address")

            const inner = ConstrData.expect(stakData.fields[0]).expectFields(1)

            if (inner.tag == 0) {
                const innerInner = ConstrData.expect(inner.fields[0]).expectFields(1)
				const innerInnerBytes = ByteArrayData.expect(innerInner.fields[0]).bytes
        ...
        if (credData.tag == 0) {
            return Address.fromPubKeyHash(new PubKeyHash(credBytes), sh, isTestnet);
        } else if (credData.tag == 1) {
            return Address.fromValidatorHash(new ValidatorHash(credBytes), sh, isTestnet);
        } else {
            throw new Error("unexpected");
        }
    }
	...
	static compare(a, b, stakingHashesOnly = false) {
		if (stakingHashesOnly) {
			if (isSome(a.stakingHash) && isSome(b.stakingHash)) {
				return ByteArrayData.compare(a.stakingHash.bytes, b.stakingHash.bytes)
			} else {
